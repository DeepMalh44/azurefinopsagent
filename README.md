...
- **Drop-in file analysis (no Azure consent required)** — drag any CSV, TSV, JSON, TXT, log, Markdown, XLSX, PDF or Parquet into the chat and click **Analyze**. The agent learns the schema, runs targeted aggregates/filters server-side (pandas/openpyxl/pyarrow/pdfminer), and answers without ever loading the raw payload into the LLM context. Perfect for ad-hoc cost exports, Advisor JSON, audit logs, or executive PDFs. See [`demo-data/`](demo-data/) for a generator (`generate.py`) and a catalog of realistic test files (~14 MB across 9 file types) you can drop in to try it.
