...
=== MONITOR (Microsoft.Insights, api-version=2023-10-01) ===
GET /{resourceId}/providers/Microsoft.Insights/metrics?api-version=2023-10-01&metricnames={name1,name2}&timespan={start}/{end}&interval={PT1H|P1D}&aggregation={Average,Maximum,Minimum,Total,Count} — query metric values for any resource. Use metricnamespace to scope (e.g. Microsoft.Compute/virtualMachines). Common FinOps metrics: VMs → 'Percentage CPU','Available Memory Bytes','Disk Read Bytes','Disk Write Bytes','Network In Total','Network Out Total'; SQL DB → 'dtu_consumption_percent','storage_percent'; Cosmos DB → 'TotalRequestUnits','NormalizedRUConsumption'; Storage → 'UsedCapacity','BlobCapacity','Transactions'; App Service → 'CpuPercentage','MemoryPercentage','Requests'; Redis → 'usedmemorypercentage','serverLoad'. Use $filter for dimensions (e.g. 'Tier eq ''Hot'''). Use resultType=Metadata to discover available dimension values. Use AutoAdjustTimegrain=true for flexibility.
GET /{resourceId}/providers/Microsoft.Insights/metricDefinitions?api-version=2023-10-01 — list all available metrics for a resource with supported aggregations, time grains, dimensions, and unit.
GET /{resourceId}/providers/Microsoft.Insights/metricBaselines?api-version=2023-10-01 — metric baselines for anomaly detection (expected low/medium/high bands).
