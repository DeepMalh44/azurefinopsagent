...
- Pick the right chart type:
    ...
    * waterfall       → cumulative impact (e.g. starting score → +tagging → +budgets → +exports → final score). Pass values as [baseline, +d1, +d2, +d3, finalTotal]. Positive deltas render green, negative red, baseline grey, total blue.
- Chart values must be NUMBERS not strings. Currency is added by the renderer (waterfall uses raw numbers, no $).
...
- 'kpi'        → 2-4 big metric cards across one row — use right after the title slide for the CFO snapshot
- 'maturity'   → the killer FinOps slide. Renders a Before/After star-rated table for the 7 Crawl dimensions plus a 'What we did' column. Use this every time you have a maturity score AND a remediation took place.
...
═══ RECOMMENDED 6-SLIDE STAKEHOLDER MATURITY-REMEDIATION DECK (use after a maturity score + fixes) ═══
This is the canonical leadership story: 'Here's where we were, what we did, and where we are now.' ALWAYS produce this exact shape after a Crawl score followed by remediation actions in the same conversation.

1. title    — '{Customer} FinOps Maturity Review' / '{Month YYYY} — Generated live from Azure'
2. kpi      — 'Where we started': 4 KPI cards — [Crawl score before (e.g. '7 / 35', accent='red'), Untagged resources, Budget coverage, Open waste $]
3. maturity — 'What we fixed in this session': rows = [{label: <dimension>, before: <0-5>, after: <0-5>, action: <one-line concrete fix with numbers>}]. Title example: 'Crawl maturity: 1.0 → 3.4 stars in 9 minutes'. Footnote with the cumulative star delta (e.g. '+17 stars across 7 dimensions').
4. chart waterfall — Title: 'How we got from {before} to {after}'. labels = ['Starting score', '+Tagging', '+Budget alerts', '+Cost exports', '+Anomaly alerts', '+Cleanup', 'New score']. values = [<baseline number>, +d1, +d2, +d3, +d4, +d5, <final number>].
5. kpi      — 'Where we are now': 4 KPI cards — [Crawl score after (accent='green'), Tagged resources delta, Active budget alerts, Annualized waste avoided $]
6. content  — 'Walk-level next steps': 3-5 bullets each starting with a verb + named owner + concrete entity (RIs, right-sizing, dev/test snoozing). End with: 'Re-score in 30 days.'

When the user (or judges in a demo) asks 'generate a leadership deck of what just happened' — use this 6-slide pattern. Do NOT improvise the order; this is the proven sequence.
");
    }
...
SLIDE OBJECT SCHEMA:
- layout: 'title' | 'section' | 'content' | 'chart' | 'two_column' | 'kpi' | 'maturity'  (REQUIRED)
...
- eyebrow: optional small uppercase label shown above the title on 'section' slides (e.g. 'PART 2')
- bullets: optional array of strings (≤5 items, ≤10 words each)
- bullets_right: optional — right column for 'two_column' layout
- kpis: REQUIRED for 'kpi' layout — array of 2-4 metric cards: [{""label"":""Monthly spend"",""value"":""$45.2K"",""sublabel"":""+18% vs last month"",""accent"":""amber""}]. accent ∈ blue|teal|green|amber|red (default cycles through palette).
- rows: REQUIRED for 'maturity' layout — array of dimension rows: [{""label"":""Tagging for accountability"",""before"":0,""after"":4,""action"":""Tagged 16 resources with CostCenter/Owner/Environment""}]. Star scores 0-5; renderer auto-colors green/blue/amber/red.
- footnote: optional bold blue summary line under the maturity table (e.g. '+17 stars across 7 dimensions in 9 minutes')
- chart: optional object {type, title, labels[], values[], colors[]?}
    * type: 'bar' | 'horizontal_bar' (preferred for top-N) | 'line' (for time series) | 'pie' (composition, ≤6 slices) | 'waterfall' (cumulative impact — values=[baseline,+d1,+d2,...,finalTotal])
...
EXECUTIVE DECK PATTERN: open with 'title' → 'kpi' (3-4 headline numbers) → 'chart' slides → 'two_column' before/after → 'content' next-steps. The 'kpi' slide right after the title is the CFO's one-glance summary — always include it for exec audiences.

GOOD EXAMPLE (note conclusion-style titles, KPI summary, takeaway bullets after charts, $ formatting):
[
  {""layout"":""title"",""title"":""Contoso Azure FinOps Review"",""subtitle"":""April 2026 — Prepared by Cloud CoE""},
  {""layout"":""kpi"",""title"":""Where Contoso stands today"",""subtitle"":""Snapshot from live Azure data, April 30 2026"",""kpis"":[{""label"":""Monthly spend"",""value"":""$45.2K"",""sublabel"":""+18% vs March"",""accent"":""amber""},{""label"":""Identified savings"",""value"":""$12.4K/mo"",""sublabel"":""27% reduction"",""accent"":""green""},{""label"":""Untagged resources"",""value"":""142"",""sublabel"":""of 387 (37%)"",""accent"":""red""},{""label"":""Open Advisor recos"",""value"":""38"",""sublabel"":""$6.8K/mo impact"",""accent"":""blue""}]},
